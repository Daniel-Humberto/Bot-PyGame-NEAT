...
def draw_network_info(surface):
    ...
    info_texts = [
        ...
        ("Population Size:", "50 (ejemplo)"),
        ("Mutation Rates:", ""),
        ("  - Weight:", "0.8 (ejemplo)"),
        ("  - Add Node:", "0.2 (ejemplo)"),
        ("  - Add Connection:", "0.5 (ejemplo)"),
...
def eval_genomes(genomes_list, config):
    ...
    while run and len(games) > 0:
        ...
        clock.tick(60)  # Ajustar FPS según sea necesario
        ...
        for i_idx in range(len(games) - 1, -1, -1):
            ...
            genome_instance.fitness += 0.1  # Recompensa por seguir vivo
...
def run_neat(config_path):
    ...
    winner = p.run(eval_genomes, 1000)  # 20 generaciones
