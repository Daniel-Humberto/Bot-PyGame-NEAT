...
generation_best_fitness = 0  # Initialize the missing variable
...
def draw_stats_panel(surface):
    ...
    alive_value = TITLE_FONT.render(f"{len(games)}", 1, POSITIVE_COLOR)
...
def draw_fitness_graph(surface):
    ...
    # FIX: Changed the color formatting to use RGB tuple for matplotlib
    plt.plot(range(1, len(generation_fitnesses) + 1), generation_fitnesses,
             color=(HIGHLIGHT_COLOR[0] / 255, HIGHLIGHT_COLOR[1] / 255, HIGHLIGHT_COLOR[2] / 255),
...
def eval_genomes(genomes, config):
    ...
    global generation, best_fitness, generation_fitnesses, all_time_best_genome, games, generation_best_fitness
    ...
    generation += 1
    generation_best_fitness = 0  # Reset for this generation
    ...
    while run and len(games) > 0:
        ...
        for i, game in enumerate(games):
            # Make sure we have pipes before proceeding
            if not game.pipes:
                continue
            ...
            # Update current generation best fitness
